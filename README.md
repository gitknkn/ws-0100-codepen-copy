...
SMACSS記述になっている
sass配下の構造
base.sass(ベース)  body, htmlの基本的な設定(背景色や縦横設定)
layout.sass(レイアウト)  header, footerなどの大枠のレイアウト
modulesフォルダー()  ボタンやナビゲーションの小枠(クラスセレクターを使用, IDや要素セレクターは避ける)
state.sass(ステート・状態)  
theme.sass(テーマ)  すべての主要なルールに対して、影響する(ボタン色やボーダー線の上書き等)
variable.sass(変数)  変数の格納先
index.sass(import先)